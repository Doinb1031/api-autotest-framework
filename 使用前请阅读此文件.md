...
**多环境配置（--env / TEST_ENV）**
------------------------------------------------------------------------------------------------
框架支持多环境切换，配置集中在 conf/config.ini：

- `[api_envi]` 是本地默认环境（http://127.0.0.1:8787 的内置 mock），同时也是所有环境的"回落基线"；
- 其他环境写成 `[api_envi:环境名]` 段，只写与默认环境有差异的配置项（没写的项自动回落到基线段）；
- 当前环境由环境变量 `TEST_ENV` 决定，可通过 pytest 参数 `--env` 覆盖，优先级：`--env` > `TEST_ENV` > 默认 local。

常用命令：

```bash
pytest                                    # 默认 local 环境（内置 mock）
pytest --env=test                         # 指定 test 环境（读 [api_envi:test] 段）
TEST_ENV=test pytest -m smoke             # 环境变量方式（CI/脚本场景）
TEST_ENV=test python run.py               # 不经过 pytest 的入口同样生效
```

新增一个环境的步骤：在 config.ini 照抄 `[api_envi:test]` 段，改名为 `[api_envi:新环境名]`，
只写有差异的配置项（如 host）。指定的环境在 config.ini 里没有对应段时，会回落到 `[api_envi]`
基线并在日志中提示，避免"以为在跑目标环境、实际在跑本地"的误判。

另：测试报告类型（allure/tm）的单一真相源也是 config.ini（`[REPORT_TYPE] type`），
环境变量 `REPORT_TYPE` 可临时覆盖，setting.py 中不再维护第二份。
