...
class DebugTalk:
    ...
    def get_extract_data(self, node_name, randoms=None) -> str:
        ...
        if randoms is not None and bool(re.compile(r'^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$').match(randoms)):
            randoms = int(randoms)
            # 【关键技巧】利用"字典字面量中重复 key 后面覆盖前面"的特性，一行实现 4 种取值逻辑
            #
            # 注意：randoms 是变量，它的值会被当作 key，而不是字符串 "randoms"
            #   - 当 randoms = 0  时，第 1 项等价于 0:  get_extract_order_data(...)
            #   - 当 randoms = -1 时，第 1 项等价于 -1: get_extract_order_data(...)
            #   - 当 randoms = -2 时，第 1 项等价于 -2: get_extract_order_data(...)
            #   - 当 randoms = 3  时，第 1 项是 3:  get_extract_order_data(...)，不与后面冲突
            #
            # 由于字典 key 唯一，后出现的字面量 key(0/-1/-2) 会覆盖前面的变量 key
            # 所以特殊值 0/-1/-2 会被后面三项"拦截"处理；其他正数则命中第 1 项按索引取值
            data_value = {
                randoms: self.get_extract_order_data(data, randoms),  # 按索引取：randoms 为 1/2/3... 时命中
                0: random.choice(data),                                # 随机取：randoms 为 0 时覆盖第 1 项
                -1: ','.join(data),                                    # 全拼字符串：randoms 为 -1 时覆盖第 1 项
                -2: ','.join(data).split(','),                          # 全转列表：randoms 为 -2 时覆盖第 1 项
            }
            # 根据 randoms 的值，从字典中取出对应的结果
